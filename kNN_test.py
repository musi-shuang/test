...
def classify0(inX, dataSet, labels, k):
    ...
    """
    # ----------------实现classify0的第一种方式：----------------
    # 1.计算距离：numpy函数shape[0]返回dataSet的行数
    dataSetSize = dataSet.shape[0]
    print("dataSetSize：" , dataSetSize)
    # tile生成和训练样本对应的矩阵，并与训练样本求差
    # b = tile(a,(m,n)):即是把a数组里面的元素列n次，然后再把数组c行复制m次，得到一个数组
    """
        tile: 3表示复制的行数， 1表示对inx的重复的次数
        In [8]: tile(inx, (3, 1))
        Out[8]:
        array([[1, 2, 3],
            [1, 2, 3],
    ...
    """ 
        欧氏距离:  点到点之间的距离
        第一行:  特征点 到 dataSet的第一个点的距离。
        第二行:  特征点 到 dataSet的第二个点的距离。
         ...
        第N行:  特征点 到 dataSet的第N个点的距离。
...
def test1():
    ...
    第一个例子演示
    """
    group, labels = createdDataSet()
    print(classify1([0.1, 0.2], group, labels, 3))
